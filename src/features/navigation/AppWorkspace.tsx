...
import { ThreadHistoryPanel } from '../thread/ThreadHistoryPanel'
import { ThreadSessionContext } from '../thread/ThreadSessionContext'
import { useThreadSession } from '../thread/useThreadSession'
import { FoundHeader } from './FoundHeader'
import '../thread/thread-entry.css'
...
export function AppWorkspace({ children }: { readonly children: ReactNode }) {
  ...
  return (
    <ThreadSessionContext value={session}>
      <div className="fixed inset-0 flex flex-col overflow-hidden bg-background-base">
          ...
          <div className="relative flex min-h-0 min-w-0 flex-1 bg-accent-black/3">
            <AnimatePresence initial={false}>
              {historyOpen ? (
                ...
                >
                  <ThreadHistoryPanel
                    threadId={inConversation ? threadId : undefined}
