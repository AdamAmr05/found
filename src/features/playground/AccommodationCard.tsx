...
export function AccommodationCard({
  ...
  return (
    <LayoutGroup id={accommodation.id}>
      <motion.article
      ...
      >
        <div className="relative h-248 overflow-hidden bg-black/4 md:h-292">
          ...
          <div className="flex items-start justify-between gap-20">
            <div>
              <button
                ...
                type="button"
              >
                <h2 className="text-title-h5 text-balance">
                  {accommodation.name}
                </h2>
            ...
            <div className="shrink-0 text-right">
              ...
              <p className="text-body-small text-foreground-muted">per month</p>
            </div>
          </div>
...
function SignalRow({ signal }: { readonly signal: RequirementSignal }) {
  return (
    <div className="flex min-h-43 items-center justify-between gap-16 py-9">
      ...
      <span className="text-right text-body-small text-foreground-muted">
        {signal.value}
      </span>
    </div>
  )
    ...
    <div>
      <div className="flex items-center justify-between rounded-8 bg-heat-8 px-12 py-10">
        ...
        <span className="font-mono text-mono-x-small text-foreground-muted">
          checked {accommodation.lastChecked}
        </span>
      </div>
      <div className="mt-10 divide-y-1 divide-black/8">
        {accommodation.evidence.map((source) => (
...
function EvidenceRow({ source }: { readonly source: EvidenceSource }) {
  return (
    <div className="grid grid-cols-[112px_1fr] gap-14 py-11">
      <div>
  ...
  return (
    <div>
      <div className="grid grid-cols-3 divide-x-1 divide-black/8 rounded-8 bg-black/4 py-12">
        <DecisionMetric
          label="All in"
          value={`€${accommodation.totalMonthly.toLocaleString('en-US')}`}
        />
        <DecisionMetric
          label="Deposit"
          value={`€${accommodation.deposit.toLocaleString('en-US')}`}
        />
