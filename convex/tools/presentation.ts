...
export const showCandidates = createTool({
  description: [
    ...
    'Do not invent missing facts. Omit unknown optional fields; use unresolved only as an evidence status.',
    'contact.email is optional: include it only when a source provides a valid email address. Otherwise omit the email field entirely, retaining any known contact name or URL. Never use placeholders such as "not provided", "unknown", "N/A", or an empty string. A missing email does not prevent presenting a candidate.',
  ].join(' '),
